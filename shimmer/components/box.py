...
class BoxDefinition:
    ...
    # Logging identifier for boxes created from this definition.
    # Defaults to the class of the box created.
...
class Box(cocos.cocosnode.CocosNode):
    ...
    def set_log_level(self, level: int) -> None:
        """Set the logging level of this Box."""
        self.logger.setLevel(level)

    def reset_log_level(self) -> None:
        """Reset the logging level of this Box to the default of NOTSET."""
        self.logger.setLevel(logging.NOTSET)

    def enable_trace_logging(self):
        """Enable trace logging for this Box which includes extra information on all logs."""
        self.set_log_level(LTRACE)

    def _add_detailed_logging(self, msg: str) -> str:
        """Add detailed logging information if this Box has trace logging enabled."""
        msg = f"[{id(self)}] {msg}"
        # Check if trace logging is enabled before we generate expensive detailed information.
        if self.logger.isEnabledFor(LTRACE):
            # Include the string representation of the state of this box in the trace log.
            msg = f"{msg}\t\t__TRACE__ {str(self)}"
        return msg

    def trace(self, msg, *args, **kwargs):
        """Log at trace level with information about the state of this Box."""
        msg = self._add_detailed_logging(msg)
        self.logger.log(LTRACE, msg, *args, stacklevel=2, **kwargs)

    def debug(self, msg, *args, **kwargs):
        """Log at debug level with extra information if trace logging is enabled."""
        msg = self._add_detailed_logging(msg)
        self.logger.debug(msg, *args, stacklevel=2, **kwargs)

    def info(self, msg, *args, **kwargs):
        """Log at info level with extra information if trace logging is enabled."""
        msg = self._add_detailed_logging(msg)
        self.logger.info(msg, *args, stacklevel=2, **kwargs)

    def warning(self, msg, *args, **kwargs):
        """Log at warning level with extra information if trace logging is enabled."""
        msg = self._add_detailed_logging(msg)
        self.logger.warning(msg, *args, stacklevel=2, **kwargs)

    def error(self, msg, *args, **kwargs):
        """Log at error level with extra information if trace logging is enabled."""
        msg = self._add_detailed_logging(msg)
    ...
    def get_z_value(self) -> Optional[int]:
        ...
        Raises AssertionError if this child is not a child of parent (should never happen).
        """
        ...
        for z, child in self.parent.children:
            if child is self:
                return z
        raise AssertionError(f"{self} is not a child of its parent.")
    ...
    def set_z_value(self, z: Union[int, ZIndexEnum]) -> None:
        ...
        if z is ZIndexEnum.top:
            ...
            if self.parent.children[0][1] is self:
                # If this box is already at the bottom, then do nothing.
                return
    ...
    def align_anchor_with_other_anchor(
        ...
        if isinstance(spacing, (cocos.draw.Point2, cocos.draw.Vector2)):
            ...
            if spacing == 0 or self_anchor == CenterCenter:
                ...
                spacing_vector *= spacing
                spacing_vector //= 1  # Round both xy to nearest int
